...
current_imgs_dir = "/mnt/sdb-seagate/graduacao/datasets/projeto_cerrado" + "/2025-06-16"
out_dir = "/mnt/sdb-seagate/graduacao/datasets/projeto_cerrado" + "/2025-06-16"
...
for _ in range(math.ceil(len(input_imgs) / slc)):
    ...
    for img_path in cur_imgs:
        ...
        if '_D.JPG' in up:
            bands['D'] = img_path
        elif '_MS_G.TIF' in up:
            bands['G'] = img_path
        elif '_MS_NIR.TIF' in up:
            bands['NIR'] = img_path
        elif '_MS_R.TIF' in up:
            bands['R'] = img_path
        elif '_MS_RE.TIF' in up:
            bands['RE'] = img_path

    if len(bands) < 5:
        print(
            f"Aviso: Grupo incompleto ignorado. Bandas encontradas: {list(bands.keys())}")
        continue
    ...
    multispectral = np.dstack(
        (ref_img, target_jpg, target_nir, target_r, target_re))
    pseudo_rgb = np.dstack((target_r, ref_img, target_nir))

    fused_image = gram_schmidt_fusion_rgb(
        multispectral=multispectral, pseudo_rgb=pseudo_rgb)

    # 4. Geração do nome de saída e Salvamento em TIF
    # Pegamos o nome da imagem original e trocamos a extensão para .TIF
    base_name_clean = os.path.basename(
        bands['D']).replace('_D.JPG', '_D_FUSED.TIF')
