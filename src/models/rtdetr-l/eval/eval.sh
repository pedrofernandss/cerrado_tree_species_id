...
LOG_DIR="/mnt/sdb-seagate/graduacao/logs/ana_pedro/evaluations/seed${SEED}/rtdetr-l"
mkdir -p "$LOG_DIR"
...
cd /mnt/sdb-seagate/graduacao/home/ana_pedro/projetos/cerrado_tree_identifier/src/models/rtdetr-l/eval/

datasets=("fused" "rgb" "ndre" "ndvi")

for NAME in "${datasets[@]}"; do
    YAML="$DATASET/$NAME/data.yaml"
    MODEL="$RUNS/seed${SEED}/rtdetr-l/${NAME}_seed${SEED}/weights/best.pt"

    if [ -f "$MODEL" ]; then
        echo "Avaliando: $NAME | seed: $SEED"
        find "$DATASET/$NAME" -name "*.cache" -delete
        $PYTHON eval.py --model "$MODEL" --data "$YAML" --seed "$SEED" --model_name "rtdetr-l" > "$LOG_DIR/$NAME.log" 2>&1
        echo "Finishing: $NAME"
    else
...
echo "yolov5n seed=$SEED avaliação concluída!"