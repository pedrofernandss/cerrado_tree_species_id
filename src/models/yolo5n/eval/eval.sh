...
LOG_DIR="/mnt/sdb-seagate/graduacao/logs/ana_pedro/evaluations/seed${SEED}/yolov5n"
mkdir -p "$LOG_DIR"
...
cd /mnt/sdb-seagate/graduacao/home/ana_pedro/projetos/cerrado_tree_identifier/src/models/yolo5n/eval/

datasets=("fused" "rgb" "ndre" "ndvi")

for NAME in "${datasets[@]}"; do
    YAML="$DATASET/$NAME/data.yaml"
    MODEL="$RUNS/seed${SEED}/yolov5n/${NAME}_seed${SEED}/weights/best.pt"

    if [ -f "$MODEL" ]; then
        echo "Avaliando: $NAME | seed: $SEED"
        find "$DATASET/$NAME" -name "*.cache" -delete
        $PYTHON eval.py --model "$MODEL" --data "$YAML" --seed "$SEED" --model_name "yolov5n" > "$LOG_DIR/$NAME.log" 2>&1
        echo "Finishing: $NAME"
    else
