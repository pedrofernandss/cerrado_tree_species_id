...
SEED=${1:-1}  # recebe seed como argumento, default 1
 
LOG_DIR="/mnt/sdb-seagate/graduacao/logs/ana_pedro/train/seed${SEED}/yolo5n"
mkdir -p "$LOG_DIR"
 
PYTHON="/mnt/sdb-seagate/graduacao/home/ana_pedro/projetos/cerrado_tree_identifier/.venv/bin/python3"
 
cd /mnt/sdb-seagate/graduacao/home/ana_pedro/projetos/cerrado_tree_identifier/src/models/yolo5n/train/
...
for ((i=0; i<${#datasets[@]}; i+=2)); do
    ...
    echo "----------------------------------------------------------"
    echo "Iniciando treino: $NAME | seed: $SEED"
    echo "----------------------------------------------------------"
...
echo "Todos os treinamentos do yolo5n (seed=$SEED) concluídos!"