...
def calculate_indexes():
    ...
    for _ in range(math.ceil(len(input_imgs)/slc)):
        ...
        rgb_image = np.asarray(Image.open(str(rgb_path))).astype(np.float32) / 255.0
        fused_image = tiff.imread(str(fused_path)).astype(np.float32)
        ...
        stack_rgb_ndvi = np.dstack((rgb_image, ndvi_image)).astype(np.float32)
        stack_rgb_ndre = np.dstack((rgb_image, ndre_image)).astype(np.float32)

        new_stem_rgb_ndvi = stem_name.replace('_D', '_RGB_NDVI')
        new_stem_rgb_ndre = stem_name.replace('_D', '_RGB_NDRE')

        tiff.imwrite(rgb_ndvi_output_dir / f"{new_stem_rgb_ndvi}.TIF", stack_rgb_ndvi)
        tiff.imwrite(rgb_ndre_output_dir / f"{new_stem_rgb_ndre}.TIF", stack_rgb_ndre)

        # Stack and save indexes with proposed Fusion
        stack_fused_ndvi = np.dstack((fused_image, ndvi_image)).astype(np.float32)
        stack_fused_ndre = np.dstack((fused_image, ndre_image)).astype(np.float32)
        
        new_stem_fused_ndvi = stem_name.replace('_D', '_FUSED_NDVI')
        new_stem_fused_ndre = stem_name.replace('_D', '_FUSED_NDRE')

        tiff.imwrite(fused_ndvi_output_dir / f"{new_stem_fused_ndvi}.TIF", stack_fused_ndvi)
